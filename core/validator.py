...
def audit_fleet_schedule(
    ...
    Checks:
    1. Battery Reserve >= 15.0% State-of-Charge for all drones.
    ...
    for route in schedule.assigned_routes:
        ...
        if route.total_energy_joules > max_allowed_energy + 1e-4:
            ...
            violations.append(
                f"Battery reserve violation on {drone.id}: consumed {route.total_energy_joules:.1f} J "
                f"(max allowed {max_allowed_energy:.1f} J). Remaining SoC: {rem_pct:.2f}% (< 15.0% floor)."
            )

        # 2. Flight Deadline Check
        ...
        # 5. Target Deconfliction Check
        route_targets = set(route.target_ids)
        overlap = visited_all_targets.intersection(route_targets)
        if overlap:
            violations.append(
                f"Target cannibalization / duplication detected: targets {overlap} visited by multiple drones."
            )
